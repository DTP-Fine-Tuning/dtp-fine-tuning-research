...
verify_project_root() {
    if [ ! -d "scripts" ] || [ ! -d "src" ] || [ ! -d "configs" ]; then
        ...
        print_info "Please cd to project root and run: ./scripts/run_training.sh"
        exit 1
    fi
}
...
EXAMPLES:
    ...
    ./scripts/run_training.sh -c configs/my_config.yaml

    # Run without W&B logging
    ./scripts/run_training.sh --no-wandb

    # Skip all checks (not recommended)
    ./scripts/run_training.sh --skip-checks
EOF
}
