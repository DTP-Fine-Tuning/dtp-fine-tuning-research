...
################################################################################
# Gradio Inference Script for Qwen Fine-tuned Model
...
BASE_MODEL=""
INFERENCE_SCRIPT="src/training/gradio_inference_qwen.py"
PORT=7860
MAX_NEW_TOKENS=512
SHARE=false
NO_4BIT=false
...
verify_project_root() {
    if [ ! -d "scripts" ] || [ ! -d "src" ] || [ ! -d "configs" ]; then
        ...
        print_info "Please cd to project root and run: ./scripts/run_inference_qwen.sh"
        exit 1
    fi
}

find_latest_qwen_model() {
    local latest_model=""
    local latest_time=0
    
    # Search for Qwen models
    for pattern in "SFT-Qwen*" "SFT-qwen*"; do
        if compgen -G "$pattern" > /dev/null 2>&1; then
            for model in $pattern/; do
                if [ -d "$model" ]; then
                    model_time=$(stat -f %m "$model" 2>/dev/null || stat -c %Y "$model" 2>/dev/null || echo 0)
...
list_available_qwen_models() {
    print_info "Available Qwen models:"
    local count=0
    ...
    if [ $count -eq 0 ]; then
        echo "  No Qwen models found. Please run training first."
        return 1
    fi
...
OPTIONS:
    -m, --model-path PATH       Path to fine-tuned Qwen model (auto-detected if not specified)
...
EXAMPLES:
    # Run with auto-detected latest Qwen model
    ./scripts/run_inference_qwen.sh

    # Run with specific model path
    ./scripts/run_inference_qwen.sh -m SFT-Qwen2-1.5B-LoRA-MultiTurn-1k

    # Run on specific port and share publicly
    ./scripts/run_inference_qwen.sh -p 8080 -s

    # Run without 4-bit quantization (requires more VRAM)
    ./scripts/run_inference_qwen.sh --no-4bit
EOF
}
...
print_header "Qwen Model Inference - Gradio Interface"
...
if [ -z "$MODEL_PATH" ]; then
    print_info "No model path specified, searching for Qwen models..."
    list_available_qwen_models
    
    MODEL_PATH=$(find_latest_qwen_model)
    if [ -z "$MODEL_PATH" ]; then
        print_error "No Qwen models found. Please train a model first or specify -m option."
        exit 1
    fi
    
    print_info "Auto-detected latest Qwen model: $MODEL_PATH"
...
if [ ! -f "$INFERENCE_SCRIPT" ]; then
    ...
    print_info "Expected location: src/training/gradio_inference_qwen.py"
    exit 1
fi
...
try:
    ...
    print(f"Chat Format: Qwen ChatML")
