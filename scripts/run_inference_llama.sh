...
################################################################################
# Gradio Inference Script for Llama Fine-tuned Model
...
BASE_MODEL=""
INFERENCE_SCRIPT="src/training/gradio_inference_llama.py"
PORT=7860
MAX_NEW_TOKENS=512
SHARE=false
NO_4BIT=false
...
verify_project_root() {
    if [ ! -d "scripts" ] || [ ! -d "src" ] || [ ! -d "configs" ]; then
        ...
        print_info "Please cd to project root and run: ./scripts/run_inference_llama.sh"
        exit 1
    fi
}
...
find_latest_llama_model() {
    ...
    for pattern in "SFT-Llama*" "SFT-llama*"; do
...
list_available_llama_models() {
    print_info "Available Llama models:"
    local count=0
    ...
    if [ $count -eq 0 ]; then
        echo "  No Llama models found. Please run training first."
        return 1
    fi
...
print_header "Llama 3.2 Model Inference - Gradio Interface"
...
if [ -z "$MODEL_PATH" ]; then
    print_info "No model path specified, searching for Llama models..."
    list_available_llama_models
    
    MODEL_PATH=$(find_latest_llama_model)
    if [ -z "$MODEL_PATH" ]; then
        print_error "No Llama models found. Please train a model first or specify -m option."
        exit 1
    fi
    
    print_info "Auto-detected latest Llama model: $MODEL_PATH"
...
if [ ! -f "$INFERENCE_SCRIPT" ]; then
    ...
    print_info "Expected location: src/training/gradio_inference_llama.py"
    exit 1
fi
