...
################################################################################
# DeepEval Evaluation Script for Qwen3 Fine-tuned Model (Gemini)
# This script evaluates the model using various metrics with Google Gemini
...
BASE_MODEL=""
EVALUATION_SCRIPT="src/training/deepeval_evaluation_gemini.py"
...
verify_project_root() {
    if [ ! -d "scripts" ] || [ ! -d "src" ] || [ ! -d "configs" ]; then
        ...
        print_info "Please cd to project root and run: ./scripts/run_evaluation_gemini.sh"
        exit 1
    fi
}
...
check_dependencies() {
    ...
    python -c "import torch; import transformers; import peft; import deepeval; import datasets; import google.generativeai" 2>/dev/null
    if [ $? -ne 0 ]; then
        print_warning "Some required packages are missing. Installing dependencies..."
        pip install torch transformers peft deepeval datasets pandas google-generativeai tenacity
    else
...
check_gemini_key() {
    if [ -z "$GEMINI_API_KEY" ] && [ -z "$GOOGLE_API_KEY" ]; then
        print_warning "GEMINI_API_KEY / GOOGLE_API_KEY not set."
        print_warning "DeepEval uses Gemini models for evaluation metrics."
        print_info "Get an API key at: https://makersuite.google.com/app/apikey"
        ...
        print_info "Gemini API key found."
    fi
}
...
OPTIONS:
    ...
    -g, --gemini-model MODEL    Gemini model to use (default: $GEMINI_MODEL)
...
EXAMPLES:
    ...
    ./scripts/run_evaluation_gemini.sh -m src/training/SFT-Qwen3-1.7B-LoRA-9GB-final -s

    # Evaluate on specific dataset
    ./scripts/run_evaluation_gemini.sh -d "izzulgod/indonesian-conversation" -n 50

    # Use Gemini Pro model
    ./scripts/run_evaluation_gemini.sh -s -g gemini-1.5-pro

NOTES:
    - DeepEval requires Gemini API key for evaluation metrics
    - Set GEMINI_API_KEY or GOOGLE_API_KEY in .env file or as environment variable
    - Using sample data (-s) is recommended for quick testing
    - gemini-1.5-flash is fastest and cheapest (recommended)
EOF
}
...
print_header "Qwen3 Model Evaluation - DeepEval (Gemini)"
...
if [ ! -f "$EVALUATION_SCRIPT" ]; then
    ...
    print_info "Expected location: src/training/deepeval_evaluation_gemini.py"
    exit 1
fi
