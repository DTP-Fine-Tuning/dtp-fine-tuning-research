...
if [ -f "run_evaluation_gemini.sh" ]; then
    chmod +x run_evaluation_gemini.sh
    print_info "run_evaluation_gemini.sh is now executable ✓"
fi
...
if [[ "$backup_choice" =~ ^[Yy]$ ]]; then
    ...
    if [ -f "deepeval_evaluation.py" ]; then
        cp deepeval_evaluation.py "$backup_dir/"
        print_info "Backed up deepeval_evaluation.py"
    fi
    
    if [ -f "run_evaluation.sh" ]; then
        cp run_evaluation.sh "$backup_dir/"
        print_info "Backed up run_evaluation.sh"
    fi
...
echo "   ./run_evaluation_gemini.sh -m ./your-model -s"
echo
echo "2. Run full evaluation:"
echo "   ./run_evaluation_gemini.sh -m ./your-model -d dataset-name -n 100"
echo
...
echo
echo "For help: ./run_evaluation_gemini.sh --help"
echo
