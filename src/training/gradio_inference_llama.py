"""
Gradio Interface for Llama Fine-tuned Model
This script provides an interactive web interface for chatting with the fine-tuned Llama model.
"""
...
class LlamaChatInterface:
    """Main class for Llama chat interface with Gradio"""
    ...
    def format_prompt(
        ...
        """
        Format the conversation history and current message into a prompt using Llama 3.2 format
        ...
        # Build conversation with Llama 3.2 format
...
def create_gradio_interface(model_path: str, **kwargs):
    ...
    Args:
        ...
        **kwargs: Additional arguments for LlamaChatInterface
        
    Returns:
        Gradio interface
    """
    # Initialize chat interface
    chat_interface = LlamaChatInterface(model_path, **kwargs)
    ...
    with gr.Blocks(title="Llama Chat Interface", theme=gr.themes.Soft()) as interface:
        gr.Markdown(
            """
            # Llama 3.2 Fine-tuned Model Chat Interface
            
            This interface allows you to interact with your fine-tuned Llama model.
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Gradio interface for Llama fine-tuned model")
    parser.add_argument(
        "--model-path",
        type=str,
        required=True,
