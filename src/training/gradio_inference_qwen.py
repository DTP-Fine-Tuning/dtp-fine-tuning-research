"""
Gradio Interface for Qwen Fine-tuned Model
This script provides an interactive web interface for chatting with the fine-tuned Qwen model.
"""
...
class QwenChatInterface:
    """Main class for Qwen chat interface with Gradio"""
    ...
    def format_prompt(
        ...
        """
        Format the conversation history and current message into a prompt using Qwen ChatML format
        ...
        # Build conversation with Qwen ChatML format
        prompt = ""
...
def create_gradio_interface(model_path: str, **kwargs):
    ...
    Args:
        ...
        **kwargs: Additional arguments for QwenChatInterface
        
    Returns:
        Gradio interface
    """
    # Initialize chat interface
    chat_interface = QwenChatInterface(model_path, **kwargs)
    ...
    with gr.Blocks(title="Qwen Chat Interface", theme=gr.themes.Soft()) as interface:
        gr.Markdown(
            """
            # Qwen Fine-tuned Model Chat Interface
            
            This interface allows you to interact with your fine-tuned Qwen model.
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Gradio interface for Qwen fine-tuned model")
    parser.add_argument(
        "--model-path",
        type=str,
        required=True,
