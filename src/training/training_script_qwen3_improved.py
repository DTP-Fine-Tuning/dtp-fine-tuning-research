...
def create_tokenizer(config: Dict) -> AutoTokenizer:
    """Create and configure tokenizer for Qwen3"""
    ...
    # Qwen3 models typically have pad_token set, but let's ensure it's properly configured
    if tokenizer.pad_token is None:
        # For Qwen3, we typically use the eos_token as pad_token
        tokenizer.pad_token = tokenizer.eos_token
        tokenizer.pad_token_id = tokenizer.eos_token_id
    
    # Ensure special tokens are properly set for Qwen3
...
def create_model(config: Dict, bnb_config: BitsAndBytesConfig) -> AutoModelForCausalLM:
    """Create and configure Qwen3 model"""
    model = AutoModelForCausalLM.from_pretrained(
        config['model']['name'],
        quantization_config=bnb_config,
        device_map="auto",
        trust_remote_code=config['model']['trust_remote_code'],
        torch_dtype=torch.float16,  # Explicitly set dtype for Qwen3
    )
...
def create_lora_config(config: Dict) -> LoraConfig:
    """Create LoRA configuration for Qwen3"""
...
def format_conversation_qwen3(example: Dict, config: Dict) -> Dict[str, str]:
    """
    Format dataset according to Qwen3 chat template with improved multi-turn handling.
    Qwen3 uses the following format:
    <|im_start|>system
    ...
    if config['chat_template'].get('use_system_message', True):
...
def format_with_tokenizer_template(
    ...
    if config['chat_template'].get('use_system_message', True):
        messages.append({
            "role": "system",
            "content": config['chat_template']['system_message']
        })
    ...
    for turn in conversation:
        ...
        # Normalize role names for Qwen3
    ...
    try:
        ...
        return format_conversation_qwen3(example, config)
...
def main(config_path: str):
    ...
    # Create tokenizer
    print("Creating tokenizer for Qwen3...")
    ...
    if hasattr(tokenizer, 'chat_template') and tokenizer.chat_template:
        ...
        print("Using manual Qwen3 chat template formatting")
        train_dataset = train_dataset.map(
            lambda x: format_conversation_qwen3(x, config),
            remove_columns=train_dataset.column_names
        )
        eval_dataset = eval_dataset.map(
            lambda x: format_conversation_qwen3(x, config),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Qwen3 model with SFT using config file")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
