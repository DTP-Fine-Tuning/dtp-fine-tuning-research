#!/usr/bin/env python3
"""
Generic SFT (Supervised Fine-Tuning) Training Script - Version 2
Uses standard Trainer from transformers with custom data collator
...
# ============================================================================
# Custom Data Collator for Instruction Masking
# ============================================================================

@dataclass
class DataCollatorForCompletionOnly:
    """
    Data collator that masks labels for instruction tokens.
    Only computes loss on the assistant's response.
    """
    ...
    def __post_init__(self):
        ...
        print(f"[INFO] Response template tokens: {self.response_template_ids}")
        if self.instruction_template_ids:
            print(f"[INFO] Instruction template tokens: {self.instruction_template_ids}")
    ...
    def torch_call(self, examples: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        for example in examples:
            ...
            response_positions = self._find_template_positions(
                input_ids,
            ...
            if response_positions:
                # Create a mask (all True initially - will be masked)
                mask = [True] * len(labels)

                # For each response position, unmask from response start to next template or end
                for pos in response_positions:
                    # Start of response (after response template)
                    response_start = pos + len(self.response_template_ids)

                    # Find end of response (next user template or end of sequence)
                    response_end = len(labels)

                    # Look for next instruction/user template
                    if self.instruction_template_ids:
                        next_inst = self._find_next_template(
                            input_ids,
                            self.instruction_template_ids,
                            response_start
                        )
                        if next_inst != -1:
                            response_end = next_inst

                    # Unmask the response tokens
                    for i in range(response_start, min(response_end, len(mask))):
                        mask[i] = False

                # Apply mask (set masked positions to ignore_index)
                labels = [
                    self.ignore_index if mask[i] else labels[i]
                    for i in range(len(labels))
                ]
            else:
                # If no response template found, mask everything (safety)
                print(f"[WARN]  Warning: No response template found in sequence, masking all tokens")
                labels = [self.ignore_index] * len(labels)
        ...
        for key in ["input_ids", "attention_mask", "labels"]:
            ...
            for seq in batch[key]:
                ...
                if key == "labels":
                    ...
                    padded_seq = seq + [self.ignore_index] * padding_length
    ...
    def _find_template_positions(self, sequence: List[int], template: List[int]) -> List[int]:
        """Find all positions where template appears in sequence"""
        positions = []
        template_len = len(template)

        for i in range(len(sequence) - template_len + 1):
            if sequence[i:i + template_len] == template:
                positions.append(i)

        return positions

    def _find_next_template(self, sequence: List[int], template: List[int], start_pos: int) -> int:
        """Find next occurrence of template after start_pos"""
        template_len = len(template)

        for i in range(start_pos, len(sequence) - template_len + 1):
            if sequence[i:i + template_len] == template:
                return i
    ...
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        ...
        if metrics is not None and "eval_loss" in metrics:
            ...
            if current_metric < self.best_metric:
                ...
                print(f"\n[BEST] New best model! Eval Loss: {current_metric:.4f}")
...
def upload_model_to_wandb(config: Dict, model_dir: str, model_family: str, artifact_name: str = None, artifact_type: str = "model"):
    ...
    if config.get('training', {}).get('report_to', 'none') == 'none':
        print("[SKIP] W&B logging disabled, skipping artifact upload")
        return

    if not os.environ.get('WANDB_API_KEY'):
        print("[WARN] WANDB_API_KEY not set, skipping artifact upload")
        return

    try:
        # Ensure wandb is initialized
        if wandb.run is None:
            print("[WARN] No active W&B run, skipping artifact upload")
            return
        ...
        print(f"\n[UPLOAD] Uploading model artifact to W&B...")
        ...
        print(f"[DONE] Model artifact uploaded successfully!")
        print(f"       View at: {wandb.run.get_url()}/artifacts")

    except Exception as e:
        print(f"[WARN] Failed to upload model artifact to W&B: {e}")
...
def setup_chat_template(tokenizer: AutoTokenizer, model_name: str, config: Dict):
    ...
    """
    if hasattr(tokenizer, 'chat_template') and tokenizer.chat_template:
        print("[DONE] Tokenizer has built-in chat template")
        return

    print("[WARN] Tokenizer missing chat template, attempting to set default...")
    ...
    if 'chat_template' in config and 'template_string' in config['chat_template']:
        ...
        print("[DONE] Applied custom chat template from config")
        return
    ...
    if family == 'llama':
        ...
        print(f"[DONE] Applied default Qwen chat template")

    else:
        print(f"[WARN] No default template for family '{family}'. Using generic template.")
        tokenizer.chat_template = (
            "{% for message in messages %}"
...
def print_trainable_parameters(model):
    ...
    print("\n" + "="*70)
    print("[INFO] TRAINABLE PARAMETERS SUMMARY")
    print("="*70)
...
def format_dataset_with_chat_template(
    ...
    try:
        ...
        print(f"[WARN] Error applying chat template: {e}")
        print(f"Messages: {normalized_messages}")
        raise


def tokenize_function(examples: Dict, tokenizer: AutoTokenizer, max_length: int) -> Dict:
    """
...
def create_training_arguments(config: Dict) -> TrainingArguments:
    """Create standard TrainingArguments"""
...
def main(config_path: str):
    ...
    # Load configuration
    print(f"\n{'='*70}")
    print(f"[LOAD] Loading configuration from: {config_path}")
    print(f"{'='*70}\n")
    ...
    if model_family:
        print(f"[INFO] Detected Model Family: {model_family}")
    else:
        print(f"[WARN] Could not auto-detect model family. Using generic settings.")
    print()
    ...
    # Create tokenizer
    print("[CREATE] Creating tokenizer...")
    ...
    print("[APPLY] Applying LoRA configuration...")
    ...
    print(f"[DONE] Train dataset tokenized: {len(train_dataset):,} examples")
    print(f"[DONE] Eval dataset tokenized: {len(eval_dataset):,} examples")
    ...
    print(f"\n[SET] Setting up instruction masking data collator...")
    ...
    # Create callbacks
    print("[SET] Setting up callbacks...")
    ...
    if torch.cuda.is_available():
        ...
        print(f"[INFO] Initial GPU Memory Available: {available_memory:.2f} GB")

    # Initialize trainer with standard Trainer
    print("\n[INIT] Initializing standard Trainer...")
    trainer = Trainer(
        model=model,
        args=training_args,
        train_dataset=train_dataset,
        eval_dataset=eval_dataset,
        data_collator=data_collator,  # Custom data collator for instruction masking
        tokenizer=tokenizer,
        callbacks=callbacks,
    )
    ...
    print("="*70)
    print(f"Trainer Type:             Standard Transformer Trainer")
    ...
    print(f"Instruction Masking:      [DONE] Enabled (Custom Data Collator)")
    print(f"Response Template:        {repr(response_template)}")
    ...
    # Train
    try:
        print("[STARTED] Starting training...\n")
        trainer.train()
    except torch.cuda.OutOfMemoryError:
        print("\n[FAILED] Out of Memory Error!")
        print("\n[TIP] Suggestions:")
    ...
    # Save final model
    print("\n" + "="*70)
    print("[INFO] Training completed! Saving final model...")
    print("="*70 + "\n")
    ...
    if config.get('save_merged_model', False):
        ...
        print(f"[SAVED] Saving merged model to: {merged_dir}")
    ...
    if trainer.state.best_metric is not None:
        print(f"\n[INFO] Best eval loss achieved: {trainer.state.best_metric:.4f}")
        if trainer.state.best_model_checkpoint:
            print(f"[SAVED] Best model checkpoint: {trainer.state.best_model_checkpoint}")

    print("\n[SUCCESS] Training pipeline completed successfully!")
    print(f"[SAVED] Model saved to: {final_model_dir}")

    # Final memory check
    if torch.cuda.is_available():
        allocated = torch.cuda.memory_allocated() / 1024**3
        reserved = torch.cuda.memory_reserved() / 1024**3
        print(f"\n[INFO] Final GPU Memory - Allocated: {allocated:.2f}GB, Reserved: {reserved:.2f}GB")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generic SFT Training Script V2 - Uses standard Trainer with custom data collator"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    if not Path(args.config).exists():
        print(f"[FAILED] Error: Configuration file not found at {args.config}")
        sys.exit(1)
